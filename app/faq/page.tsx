...
This strategy measures momentum across 19 sectors (17 sector/commodity ETFs + BTC + ETH) — 16 weeks for Balanced mode, 26 weeks for Growth mode. The top 1–2 sectors must also pass an EMA 10/100 daily trend filter — we only enter sectors in confirmed uptrends. Balanced mode rebalances weekly, Growth mode rebalances monthly.`,
      },
      {
        q: 'What are the 19 sectors?',
        a: `Each sector is a broad market segment tracked by a well-known ETF or direct asset:
...
All 19 sectors compete in the same momentum ranking each rebalance. The strategy holds the top 1–2 that pass the EMA 10/100 daily trend filter.`,
      },
      {
        q: 'What is the EMA 10/100 trend filter?',
        a: `Before entering any sector, the strategy checks whether it is in a genuine uptrend using two exponential moving averages on daily price data:
...
This alignment means the asset is rising on both a short and medium-term basis — not just making a short-lived spike. If a top-ranked sector fails this test (e.g. BTC during a crypto winter), it is skipped and the next qualifying sector is used instead. If no sectors qualify, the strategy holds cash.
...
— For equity sector or commodity sectors: the position is held via the sector ETF directly (e.g. buy XLK for Technology, GLD for Gold)
— For crypto sectors: Bitcoin (BTC) or Ethereum (ETH) are held as direct assets — not futures or proxies

There are two modes:
• Balanced (recommended): holds top 2 qualifying sectors — 50% each. 47.72% CAGR, 1.163 Sharpe, -46.4% max drawdown. Rebalances weekly.
• Growth: holds only the single top-ranked sector — 100% concentration. 61.34% CAGR, 1.065 Sharpe, -75.9% max drawdown. Rebalances monthly.`,
      },
      {
...
Structural simplicity: The strategy has very few parameters — lookback period, number of sectors, EMA spans, rebalance cadence. The fewer parameters you tune, the less room there is to overfit.
...
That said, all backtests are partially optimistic. Transaction costs, execution timing, and the fact that history doesn't repeat exactly will cause real-world results to differ. The 47.72% CAGR (Balanced) is a historical reference point, not a guarantee.`,
      },
      {
        ...
        a: `Fair challenge. Parameters weren't chosen first and validated second — they were selected from a full grid search across lookback periods (12, 16, 20, 26 weeks), EMA combinations (fast: 10/20/50/60, slow: 100/150/200/250), rebalance frequencies (7, 30, 90 days), and concentration (top 1 or 2 sectors).
...
No parameter selection process eliminates overfitting entirely. What it can do is reduce the risk by requiring robustness across a range, which this strategy passes.`,
      },
      {
        ...
        a: `BTC and ETH have historically been among the highest-momentum assets during their bull cycles — and they compete fairly in the same ranking as all other sectors. If crypto has stronger momentum than Technology or Gold and is in a confirmed uptrend, it earns an allocation.

Critically, the EMA 10/100 daily trend filter prevents allocation during crypto bear markets. BTC fell below its 100-day EMA during 2018–2019 and 2022–2023 — the strategy correctly held other sectors during those periods, avoiding the worst drawdowns.
...
This is not speculative crypto exposure. It is rules-based: crypto only gets capital when it is objectively in a strong, confirmed trend.`,
      ...
      {
        q: 'Why not just hold BTC and ETH permanently instead of rotating through 19 sectors?',
...
The rotation strategy solves this problem by applying the EMA 10/100 daily filter. When BTC falls below its 100-day EMA — which it did in both major crypto bear markets — the strategy exits and rotates into whatever sector is actually in a confirmed uptrend at that time. It may be Technology, Gold, Energy, or even cash. The strategy does not hold crypto in a downtrend simply because it was strong before.
...
Holding BTC/ETH permanently also concentrates 100% of the portfolio in two highly correlated assets with near-identical bull and bear cycles. The 19-sector universe provides signal diversity — in periods when crypto is in a bear market, equity sector or commodity momentum still exists, and the strategy captures it.
...
First, idiosyncratic risk: individual stocks can crater 50–80% on a single earnings miss, fraud allegation, or sector rotation — events that have nothing to do with the broader sector momentum you are trying to capture. An ETF averages this away.

Second, stock selection adds noise. Momentum works at the sector level — the signal that Gold is outperforming Energy is clean and persistent. Trying to pick which gold miner outperforms the others adds a layer of uncertainty that degrades the signal.
...
The conclusion from the data is unambiguous: more granular does not mean better. ETF-level rotation is cleaner, more liquid, and more profitable than stock picking within sectors.`,
      },
      {
...
Concentration risk: Holding 1–2 sectors means the portfolio is not broadly diversified. A sudden reversal in the held sector hits the full portfolio.
...
If you already trade using technical analysis, fundamentals, or your own system, the momentum rankings act as a quantitative confirmation layer. Before entering a trade, check whether the sector ranks in the top 5 with an uptrend confirmed. If it does, you have data-driven backing for your view. If it ranks 15th of 19, you know you are fighting the macro trend.
...
2. Note the current sectors and their target weights (50% each for Balanced mode)
...
For BTC/ETH: any regulated crypto exchange, or a spot Bitcoin/Ethereum ETF as a proxy if preferred.`,
      },
      {
        ...
        a: `Missing by a day or two is fine — don't stress it. The momentum signals that drive sector selection shift gradually. The sector ranked #1 today will almost always still rank #1 tomorrow.
...
One practical note: keep a 1–2% cash buffer in your account for transaction costs and to avoid being fully invested (which can cause issues on settlement days).`,
      },
      {
        ...
        a: `Cash periods occur when no sectors pass the EMA 10/100 daily trend filter — meaning no asset class is in a confirmed uptrend that meets the entry criteria. Rather than force a position into a weak or deteriorating market, the strategy steps aside entirely. Historically this has happened several times over 19 years.

During a cash period, the dashboard will show that no sectors are currently selected. Check back at the next scheduled rebalance date — the strategy will re-enter as soon as qualifying sectors emerge.
